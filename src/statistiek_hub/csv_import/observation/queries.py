create_table_as_query = (
    "CREATE TABLE IF NOT EXISTS {tmp_table_name} AS TABLE {table_name} WITH NO DATA"
)
...
insert_query = (
    ...
    "SELECT tmp.value, tmp.measure_id, tmp.spatialdimension_id, tmp.temporaldimension_id,now() AS created_at, now() AS updated_at "
