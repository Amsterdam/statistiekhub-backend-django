                ...
                begin
                    ...
                    if p_total > 0 then
                        ...
                        foreach i in array p_rule loop -- start loop rule
                            ...
                            if i like '$%' then -- start measure
                                ...
                                p_stmt_with := 	p_stmt_with || 'var' || p_number ||
                                                ' as	(
                                                        ...
                                                        , 		o.value
                                                        from	(
																select * from statistiek_hub_observation
																union all
																select * from statistiek_hub_calcobservation
																) o
                           ...
                           -------------------------------------------------------
		                        ...
		                        p_bracket_number := p_bracket_number - 1; -- count closing brackets
		                       	...
		                       	if p_bracket_number = 0 then
		                          	p_stmt_value := p_stmt_value || i || ' then ' || p_replacement_value || ' else cast(var' || p_number || '.value as float) end';
		                        end if;
                        ...
                        p_stmt_select :=	'
                                            ...
                                            ,       now() as update_at
                                            ,       cast(row_number() over () as integer) as local_id
