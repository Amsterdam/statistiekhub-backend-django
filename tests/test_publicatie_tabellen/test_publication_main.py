...
def fill_ref_tabellen() -> dict:
    ...
    return {'unit': unit, 'theme': theme, 'temp': temp, 'temp2': temp2, 'temp2_startdate': temp2_startdate, 'spatial': spatial}
...
def test_fill_observationcalculated(fill_ref_tabellen, calculation, var1_value, base_value, expected):
    ...
    # make measures
    measure_calc = baker.make(Measure, name='CALCVAR', calculation= calculation, unit=ficture['unit'])
    measure_base = baker.make(Measure, name='BASE', unit=ficture['unit'])
    measure_var1 = baker.make(Measure, name='VAR1', unit=ficture['unit']) 
    
    # make observations
    obs_base = baker.make(Observation, measure=measure_base, temporaldimension=ficture['temp'] , spatialdimension=ficture['spatial'] , value=base_value)
    obs_var1 = baker.make(Observation, measure=measure_var1, temporaldimension=ficture['temp2'] , spatialdimension=ficture['spatial'] , value=var1_value)
